...
void UMGPGameplayAbility_RangeWeapon::StartRangeWeaponTargeting()
{
	...
	if ( HitResults.Num() > 0 )
	{
		...
		{
			FMGPGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FMGPGameplayAbilityTargetData_SingleTargetHit();
...
void UMGPGameplayAbility_RangeWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InTargetData, FGameplayTag InTag)
{
	const UAbilitySystemComponent* AbilitySystemComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	...
	{
		const FGameplayAbilityTargetDataHandle LocalTargetDataHandle( MoveTemp( const_cast<FGameplayAbilityTargetDataHandle&>( InTargetData ) ) );
